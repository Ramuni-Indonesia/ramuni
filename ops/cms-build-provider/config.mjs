...
const secret = (env, name) => { const path = absolute(env, name); const info = statSync(path); if (!info.isFile() || (info.mode & 0o077) !== 0) throw new Error(`${name} must be a protected file`); const value = readFileSync(path, 'utf8').trim(); if (value.length < 32) throw new Error(`${name} is invalid`); return value; };
...
export function loadConfig(env = process.env) {
  ...
  return {
    ...
    cmsBaseUrl: cmsBaseUrl.toString(), publicBaseUrl: publicBaseUrl.toString(), deliveryToken: secret(env, 'RAMUNI_PROVIDER_CMS_DELIVERY_TOKEN_FILE'),
    sharedSecret: secret(env, 'RAMUNI_PROVIDER_SHARED_HMAC_SECRET_FILE'),
