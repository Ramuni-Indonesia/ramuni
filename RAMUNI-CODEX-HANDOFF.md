...
## Production GSC-led editorial expansion release, 2026-08-21

- Published source commits: `8125a10d0883` (`content: expand GSC-led editorial coverage`) and `dda8a35c4bf8` (`fix(content): normalize editorial punctuation`). The active atomic production release is `20260821T154214Z-dda8a35c4bf8`; artifact SHA-256 `2449f5f3d7e2dbc4feef1c3277513b0ffeec3b545f2fbfb9b4dda16e892201b4`.
- The prior 9-18 August Search Console baseline was treated as directional, early query-and-landing-page evidence rather than a volume claim. It informed three separate practical intents: `/blog/cara-membuat-laporan-penjualan-bulanan/`, `/blog/cara-menghitung-laba-bersih-usaha/`, and `/blog/contoh-laporan-stok-barang-untuk-toko/`.
- The three earlier GSC-led landing pages received genuine contextual improvements and material update dates: `/blog/contoh-laporan-penjualan-harian/` now bridges into the monthly report, `/blog/biaya-tetap-dan-biaya-variabel/` into profit calculation, and `/blog/penyebab-selisih-stok-dan-cara-mengeceknya/` into the stock report. Each retains a distinct search intent rather than duplicating the new article.
- The new financial and inventory explanations use disclosed Ikatan Akuntan Indonesia SAK EMKM references, stay educational rather than tax/accounting advice, and attach only existing approved resource, product, template, and calculator paths. No new raster generation or asset upload was required.
- Production checks passed: Node 22 Astro check (137 files, 0 errors/warnings/hints), production build (264 pages), post-CDN full site audit, content-gateway tests (14/14), keyword-map audit (5,000 keywords/100 groups), `git diff --check`, and `npm audit --audit-level=high` (0 vulnerabilities).
- Each new public article was checked live for `index,follow`, its HTTPS self-canonical, `hreflang="id-ID"`, `hreflang="x-default"`, and its 21 August 2026 date. The three updated articles also display the real material-update date. `robots.txt` declares the public sitemap.
- Public sitemap verification passed after activation: root index `6/6`, pages `12/12`, products `1/1`, solutions `1/1`, industries `1/1`, resources `40/40`, blog index `2/2`, blog posts `106/106`, and blog authors `4/4` `<loc>/<lastmod>` pairs. All six affected blog entries carry `2026-08-21` in `sitemap-blog-posts.xml`.
- One pre-activation audit correctly rejected typographic dash characters in the three new Markdown files; they were normalized and revalidated. A subsequent deployment attempt timed out while R2 performed unchanged-object checks, before activation. An earlier successful R2 pass had reported `0 uploaded, 210 unchanged`; because this release introduces no public assets and reuses already-published covers, the final exact-commit deployment safely skipped that redundant network check. Its build, audit, compression, atomic switch, and health verification all passed.

## Production Ahrefs, Clarity, and audit-remediation release, 2026-08-22

- Published runtime commit: `0cb066484fdb725d279e430de8b2fb27c47ba362` (`fix(seo): install ahrefs and resolve audit findings`). Atomic production release: `20260822T082221Z-0cb066484fdb`; artifact SHA-256: `a893e2053752cd78681b83691d8fccc29b241cac72478d1c0d87e94fdbe0cb6c`.
- Ahrefs Analytics is installed in `BaseLayout.astro` with the supplied key and `async` loading. It is intentionally production-only; preview and staging builds do not send the analytics request.
- Microsoft Clarity no longer adds its package runtime to the initial bundle. `ClarityTracking.astro` now uses the existing consent payload and loads the tag asynchronously during `requestIdleCallback` (or after a two-second fallback), reducing initial-render and PSI Best Practices pressure without changing the consent defaults.
- The five public Ahrefs exports from the supplied Drive folder were processed: six short meta descriptions were expanded, the `/tentang/` and `/kontak/` titles were made descriptive, stale `/produk/asisten-ai/` noindex data was verified against the current indexable production page, the 25 broken external references were replaced with current IAI/NIST sources, and the JSON-LD output was normalized from a top-level array to one Schema.org `@context`/`@graph` document for validator compatibility.
- Sitemap policy now includes a real static `lastmod` for `/keamanan/`; noindex routes remain excluded. The published sitemap contains no stale broken references, and the live `/produk/asisten-ai/` page remains `index,follow` while `/keamanan/` remains intentionally noindex.
- Verification passed: Node 22 Astro check (137 files, 0 errors/warnings/hints), production build (264 pages), post-CDN asset rewrite, full site/SEO/schema/internal-link/noindex/robots/accessibility audit, keyword audit (5,000 keywords/100 groups), `git diff --check`, and `npm audit --audit-level=high` (0 vulnerabilities). R2 reported `0 uploaded, 210 unchanged`.
- Live smoke checks passed for `/healthz`, representative public routes, `/sitemap.xml`, and `/sitemap-pages.xml`. The health endpoint reports the release above; the checked article has one Ahrefs script/key, idle Clarity loading, self-canonical, both hreflang links, valid robots metadata, and zero stale audit URLs.
