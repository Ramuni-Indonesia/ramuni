...
test('flags any non-zero stock-opname difference for review', () => {
  const result = calculateBusinessMetric('selisih-stok', { bookStock: 85, physicalStock: 82 });
  assert.equal(result.value, -3);
  assert.equal(shouldUseCautionNote('selisih-stok', result.value), true);
});
